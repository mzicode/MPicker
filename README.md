...
MPicker 不会在库 Manifest 中自动声明媒体读取、相机、录音、网络或所有文件访问权限。宿主 App 应根据业务场景自行声明。框架在申请运行时权限前会检查宿主 Manifest；如果缺少声明，会在 Logcat 中提示并跳过权限申请，避免线上因库隐式权限导致审核或隐私问题。
...
媒体权限、相机权限、所有文件访问权限都可能影响上架审核和隐私披露。MPicker 选择让宿主 App 显式声明需要的权限，避免库隐式带入不必要权限。
