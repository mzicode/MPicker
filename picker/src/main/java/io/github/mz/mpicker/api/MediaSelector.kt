...
import io.github.mz.mpicker.compress.MediaCodecVideoCompressor
import io.github.mz.mpicker.compress.SmartImageCompressor
import io.github.mz.mpicker.loader.DefaultImageEngine
...
/**
 * MPicker 的链式选择器实现。推荐从 [MPicker.with] 创建：
 *   MPicker.with(activity)
 ...
 *
 * Fragment 中可直接使用：
 *   MPicker.with(fragment)
 *     .type(MediaType.IMAGE)
 *     .start { result -> ... }
 *
 * 初始化预查询：
 *   MPicker.preload(context, MediaType.IMAGE, MediaType.VIDEO)
 */
...
/** MPicker 品牌入口；Java/Kotlin 都可直接写 `MPicker.with(...)`。 */
object MPicker {
    @JvmStatic
